...
client.on :message do |data|
  ...
  when 'Wie wird das Wetter Bonn?' ,  'Wie ist das Wetter in Bonn?' then
    ...
    client.message channel: data['channel'], text: wetterinfo['weather'][0]['description']
     ...
     when 'Wie wird das Wetter in Barcelona?' ,  'Wie ist das Wetter in Barcelona?' then
    wetterinfo = Net::HTTP.get('api.openweathermap.org', '/data/2.5/weather?q=Barcelona&appid=b1b15e88fa797225412429c1c50c122a')
    wetterinfo = JSON.parse wetterinfo
    client.message channel: data['channel'], text: wetterinfo['weather'][0]['description']
    
    when 'Wie wird das Wetter in Antartik?' ,  'Wie ist das Wetter in Barcelona?' then
    wetterinfo = Net::HTTP.get('api.openweathermap.org', '/data/2.5/weather?q=Barcelona&appid=b1b15e88fa797225412429c1c50c122a')
    wetterinfo = JSON.parse wetterinfo
    client.message channel: data['channel'], text: wetterinfo['weather'][0]['description']
    ...
    client.message channel: data['channel'], text: wetterinfo['weather'][0]['description']
     ...
     when 'Wie wird das Wetter in NewYork?' ,  'Wie ist das Wetter in NewYork?' then
    wetterinfo = Net::HTTP.get('api.openweathermap.org', '/data/2.5/weather?q=NewYork&appid=b1b15e88fa797225412429c1c50c122a')
    wetterinfo = JSON.parse wetterinfo
    client.message channel: data['channel'], text: wetterinfo['weather'][0]['description']
